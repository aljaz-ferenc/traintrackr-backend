...
export async function PUT(request: NextRequest, {params}: { params: Promise<{ mesoId: string }> }) {
    try {
        await connectToDatabase();
        const {mesoId} = await params
        const newMeso = await request.json();
        const updatedMeso = await MesocycleModel.replaceOne({_id: mesoId}, newMeso)

        return NextResponse.json({data: updatedMeso}, {
            status: 200, headers: {
                "Access-Control-Allow-Origin": "*",
            }
        });
